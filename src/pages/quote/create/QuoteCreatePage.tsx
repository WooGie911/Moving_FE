...
const QuoteCreatePage = () => {
  ...
  const handleConfirmQuote = useCallback(async () => {
    ...
    try {
      ...
      const response = await submitQuote(quoteRequest);
  ...
  // 견적 요청 데이터 생성
  const createQuoteRequest = (): IQuoteRequest => {
    return {
  ...
  // 견적 제출
  const submitQuote = async (quoteRequest: IQuoteRequest) => {
    if (isEditMode) {
      return await QuoteService.updateActiveQuote(quoteRequest);
    } else {
      return await QuoteService.createQuote(quoteRequest);
    }
  };
