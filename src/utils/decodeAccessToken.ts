...
export async function decodeRefreshToken(token: string): Promise<DecodedTokenPayload | null> {
  if (!token) return null;

  try {
    const { payload } = await jwtVerify(token, new TextEncoder().encode(process.env.JWT_REFRESH_SECRET_KEY));
