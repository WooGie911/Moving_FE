...
export const Tab = ({ tabList, className = "" }: ITabProps) => {
  ...
  return (
    <div className={`border-border-light flex w-full justify-center border-b-[1px] px-6 ${className}`}>
      <nav 
        className="flex w-[var(--breakpoint-lg)] gap-10 overflow-x-auto"
        aria-label={t("aria.tabNavigation")}
        role="tablist"
      >
        {tabList.map((tab, index) => {
          const isActive = cleanPath === tab.href;
          return (
            <Link
              key={tab.name}
              href={tab.href}
              ...
              aria-label={`${tab.name} ${isActive ? t("aria.activeTab") : t("aria.inactiveTab")}`}
