...
export const FormField = ({
  ...
  // ✅ 글자 수 제한 분기
  const maxLength = name === "shortIntro" ? 30 : name === "detailIntro" ? 300 : undefined;

  return (
    <fieldset className={`flex flex-col gap-4 ${className}`} aria-label={t("aria.formFieldLabel", { field: label })}>
      <legend className="flex w-full items-center justify-between gap-1">
        ...
        {/* ✅ 글자 수 표시 조건 */}
        {maxLength && (
          <span className={`ml-2 text-sm ${value?.length === maxLength ? "text-red-500" : "text-gray-500"}`}>
